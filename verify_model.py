...
def main(model_path, classes_path, image_size):
    ...
    for label in ["autorickshaw", "car", "van"]:        
        ...
        pred_arg = np.argmax(output[0])
        pred_conf = output[0][0][pred_arg]
        pred_conf = str(round(pred_conf, 2))
        pred_cls = classes[pred_arg]
        pred = f"{str(pred_cls)} {pred_conf}"
...
# python3 verify_model.py -m "models/model.onnx" -c "sample_classes.txt" -s 180